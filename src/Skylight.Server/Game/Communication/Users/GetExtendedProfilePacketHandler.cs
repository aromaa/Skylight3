	...
	where T : IGetExtendedProfileIncomingPacket
{
	...
	internal override void Handle(IUser user, in T packet)
	{
		...
		user.Client.ScheduleTask(async client =>
		{
			...
			client.SendAsync(new ExtendedProfileOutgoingPacket(new ExtendedProfileData(userId, profile.Username, profile.Figure, profile.Motto, "08-03-2001", 666, 0, false, false, true, new List<string>(), (int)(DateTime.Now - profile.LastOnline).TotalSeconds, true, false, 69, 8, 420, true, false)));
		});
	}
}
