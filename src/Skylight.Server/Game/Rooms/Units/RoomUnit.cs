...
internal sealed class RoomUnit : IUserRoomUnit
{
	...
	public void Chat(string message, int styleId = 0, int trackingId = -1)
	{
		...
		this.Room.SendAsync(new ChatOutgoingPacket(this.Id, this.User.Info.Username, message, 0, styleId, trackingId, Array.Empty<(string, string, bool)>()));
	}
	...
	public void Shout(string message, int styleId = 0, int trackingId = -1)
	{
		...
		this.Room.SendAsync(new ShoutOutgoingPacket(this.Id, this.User.Info.Username, message, 0, styleId, trackingId, Array.Empty<(string, string, bool)>()));
	}
	...
	private bool TriggerOnSayWired(string message, int styleId = 0, int trackingId = -1)
	{
		...
		this.User.SendAsync(new WhisperOutgoingPacket(this.Id, this.User.Info.Username, message, 0, styleId, trackingId, Array.Empty<(string, string, bool)>()));
