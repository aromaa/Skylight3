	...
	: IUserAuthentication
{
	...
	public async Task<IUser?> LoginAsync(IClient client, int userId, CancellationToken cancellationToken = default)
	{
		...
		Purse purse = Purse.FromDatabase(profile.Id, dbContext, cancellationToken);
		...
		User user = new(this.roomManager, client, profile, purse, new UserSettings(userSettings));
