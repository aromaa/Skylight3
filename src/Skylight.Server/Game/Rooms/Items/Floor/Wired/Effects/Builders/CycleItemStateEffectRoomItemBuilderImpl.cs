...
internal sealed class CycleItemStateEffectRoomItemBuilderImpl : FloorRoomItemBuilder
{
	private ICycleItemStateEffectFurniture? FurnitureValue { get; set; }

	private JsonDocument? ExtraDataValue { get; set; }

	public override FloorRoomItemBuilder Furniture(IFloorFurniture furniture)
	{
		this.FurnitureValue = (ICycleItemStateEffectFurniture)furniture;
	...
	public override IFloorRoomItem Build()
	{
		...
		return new CycleItemStateEffectRoomItem(this.RoomValue, this.ItemIdValue, this.OwnerValue, this.FurnitureValue, this.PositionValue, this.DirectionValue, handler, null, this.ExtraDataValue)
		{
			EffectDelay = 0
		};
	}
